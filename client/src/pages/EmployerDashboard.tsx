...
const EmployerDashboard = () => {
  ...
  window.addEventListener('applicationSubmitted', (e: any) => {
    setNotifications((prev: any[]) => [
      { id: Date.now(), message: `New application submitted for job: ${e.detail?.jobTitle || 'Unknown'}`, timestamp: Date.now() },
      ...prev,
    ]);
  });
  ...
  const saveEditJob = (jobId: number) => {
    setJobs(jobs.map(job => job.id === jobId ? { ...job, title: editTitle } : job));
    setEditJobId(null);
    setEditTitle("");
  };
  ...
  const updateApplicantStatus = (jobId: number, applicantId: number, status: string) => {
  ...
  const onSubmit = (data: any) => {
    ...
    setNotifications([
      { id: Date.now(), message: `New job posted: ${data.title}`, timestamp: Date.now() },
      ...notifications,
    ]);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 flex flex-col">
      ...
      <div className="flex flex-1 max-w-7xl mx-auto w-full">
        <SidebarProvider className="h-full">
          <Sidebar className="bg-white border-r w-56 min-h-full py-10 px-2 mt-[35px]">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Textarea placeholder="List the requirements..." {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="e.g. $120k - $160k" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Select value={typeof field.value === 'string' ? field.value : 'open'} onValueChange={field.onChange}>
                            <SelectTrigger>
                    ...
                    <React.Fragment key={job.id}>
                      <tr className="border-b">
                        ...
                        <td className="py-2 px-4 flex gap-2">
                          {editJobId === job.id ? (
                            <>
                              <Button size="sm" onClick={() => saveEditJob(job.id)}>
                                Save
                              </Button>
                              <Button size="sm" variant="outline" onClick={() => setEditJobId(null)}>
                                Cancel
                              </Button>
                            </>
                          ) : (
                            <>
                              ...
                              <Button size="sm" onClick={() => startEditJob(job)}>
                                Edit
                              </Button>
                              ...
                              {job.applicants.length === 0 ? (
                                ...
                                <ul className="space-y-2">
                                  {job.applicants.map((applicant) => (
                                    <li key={applicant.id} className="flex items-center gap-4">
            ...
            <div className="w-full max-w-2xl bg-white rounded-lg shadow-md p-6">
              ...
              <ul className="space-y-3">
                {notifications.length === 0 ? (
                  <li className="text-gray-500 text-center py-4">No notifications yet.</li>
                ) : (
                  notifications.map((n) => (
                    <li 
                      key={n.id} 
